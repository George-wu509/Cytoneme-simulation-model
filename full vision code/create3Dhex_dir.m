...
p.PF=2;   % probability of formation function: (1. const),(2. PF const/length), (3. function of averaged gradient),(4. d-effect)(5. inverse gradient)(6. it jt concentration)
p.MA=1;     % morphogen transport amount function: (1. one cytoneme), (2. N cytoneme)
p.CA=1;     % cytoneme capping criteria

p.amount=1;    % transport morphogen through cytoneme [C/sec] (## no data!!)
p.theta=1; % p.theta=1~12(degree: 0~330) if angle fun=1
p.length=40; % length of const cytoneme value if length fun=1 (## Dpp_avg:20um [242], Hh_avg: 27um in P, 13um in A [26])
p.prob_format=0.1; % probability of cytoneme formation. if prob fun =1 (## estimated!)
p.stop_format=1; % probability of cytoneme stop. if prob fun =1 (## estimated!)
p.gradient_L=10; % cytoneme length gradient coefficient. if length fun=3, length=p.gradient_L* averaged gradient(## estimated!)
p.gradient_P2=100; % cytoneme probability gradient coefficient. if probability fun=2, p_format=p.gradient_P*agrad; (## estimated!)
...
p.gradient_P5=3; % cytoneme probability gradient coefficient. if probability fun=3, p_format=p.gradient_P*agrad; (## estimated!)
...
p.capping=0; % filipodia capping check
...
% 1. mesh parameters
p.nx=160;   % wing disc nx=120(167um) (## actual width: 223(300um)[242])
p.ny=50;   % wing disc ny=50 (83um)  (## actual width: 111(150um)[242]) 
...
p.sta_save_period=10;     % figures and record every generations
p.area_rx=10;     % search space x radius (## Dpp:40um =2*Dpp_avg [242],Hh:54um in P,28um in A [26])
...
% 3. figure
p.img=0;              % no run any figures sub function
p.save_every_img=0;   % output figures for each display step. 1: output figures, 0: no output figures
p.show_every_img=0;   % show figures for each display step. 1: output figures, 0: no output figures
...
if isempty(angle_g{thi})==1
    ...
    if p.capping==1
        length=capping_length(angle_gL{thi},length,p);
    end
...
function j2=j_order(jmax_o,w_dppcent)
del_list=[72:80];
jmax=jmax_o-w_dppcent;
if mod(jmax,2)==0
    C=jmax_o:-1:jmax_o-w_dppcent+1;
    A=1:jmax/2;B=jmax:-1:jmax/2+1;
else
    C=jmax_o:-1:jmax_o-w_dppcent+1;
    A=1:(jmax+1)/2;B=jmax:-1:(jmax+1)/2+1;
end
if isempty(del_list)~=1
for k=1:size(del_list,2)
    A=A(A~=del_list(k));
    B=B(B~=del_list(k));
end
end
...
ai=1;bi=1;j2=[];
for i=1:size(A,2)+size(B,2)
    if mod(i,2)==1&&ai<=size(A,2)
        j2=[j2 A(ai)];
        ai=ai+1;
    elseif bi<=size(B,2)
        j2=[j2 B(bi)];
